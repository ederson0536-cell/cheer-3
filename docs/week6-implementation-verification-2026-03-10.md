...
5. 剩余缺口收口
   - 发布门硬约束：`governance.publish()` 接入 regression gate，`fail` 直接阻断，`warning` 无人工 token 阻断。
   - Proposal 主链路兼容：`proposal_processor` 增加 `add_proposal(...)` 兼容接口，修复 runtime 调用断链。
   - 文件目录库口径对齐：`build_file_catalog_db.py` 改为 path-based `file_id`，与 runtime `file_governance` 一致。
   - 增加 ownership lock：`transactional_patch_apply` 写前加锁、写后释放，冲突时阻断。
