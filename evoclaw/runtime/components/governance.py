...
class GovernanceGate:
    ...
    def _load_release_gate_status(self) -> Dict:
        report_file = WORKSPACE / "evoclaw" / "runtime" / "examples" / "regression_report.json"
        if not report_file.exists():
            return {"status": "warning", "action": "manual_review_required", "reason": "missing_regression_report"}
        try:
            report = json.loads(report_file.read_text(encoding="utf-8"))
        except Exception:
            return {"status": "warning", "action": "manual_review_required", "reason": "invalid_regression_report"}
        gate = report.get("release_gate", {})
        return {
            "status": gate.get("status", "warning"),
            "action": gate.get("action", "manual_review_required"),
            "policy_version": gate.get("policy_version"),
        }

    def publish(self, proposal_id: str, manual_review_token: str | None = None) -> bool:
        """Publish approved proposal."""

        if self._in_freeze_window():
            return False

        gate = self._load_release_gate_status()
        status = str(gate.get("status", "warning")).lower()
        if status == "fail":
            return False
        if status == "warning" and not manual_review_token:
            return False
