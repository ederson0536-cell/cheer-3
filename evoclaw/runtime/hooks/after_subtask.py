...
def run_after_subtask(
    ...
    ingest_week4_subtask_record(subtask_id, summary)
...
def ingest_week4_subtask_record(subtask_id: str, summary: dict):
    lifecycle = get_memory_lifecycle()
    lifecycle.ingest({
        "record_type": "episodic",
        "memory_status": "episodic",
        "schema_version": summary.get("schema_version", "v1"),
        "policy_version": summary.get("policy_version", "v1"),
        "task_id": summary.get("parent_task_id"),
        "subtask_id": subtask_id,
        "content": summary.get("goal") or summary.get("error_message") or "",
        "source_hook": "after_subtask",
    })
